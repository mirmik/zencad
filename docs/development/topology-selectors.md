...
The private typed API exposes every topology query as a composable
...
- `sort_by(Axis.Z)` orders by center projection and preserves input order for
  ties.
...
- `largest()` returns the first maximum by linear, surface, or volume measure.
...
Topology query order remains the established ZenCad typed contract.
`vertices()` is unique by OCCT `IsSame`; other queries retain
`TopExp_Explorer` occurrences, including repeated shared subshapes. Selectors
preserve that order and those occurrences. This is deterministic for one BREP
traversal but is not persistent topological naming across modeling changes.
