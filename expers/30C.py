...
from termin.algo import linear_solve
...
signal_center = assemble.unit(location=up(5))
signal_center.add_triedron()
...
chain = termin.kinchain.KinematicChain3(
	distal=signal_center.transform,
	proximal=slider_act.transform)
...
def animate(wdg):
	...
	J0 = chain.sensitivity_jacobian()

	#U = numpy.array([0,0,0,0,-0.1,0])
	W = numpy.array([-0.01,0,0,0,0,0])
	
	Ut = (
		+ numpy.array([0,0,55]) 
		- signal_center.transform.global_pose().lin
	) * 0.01
	U = numpy.array([-0.01,0,0,Ut[0],Ut[1],Ut[2]])
	...
	chain.apply_coordinate_changes(x)
	base.update_location_from_transform()
