...
def _direction(shape: ResolvedShape, *, planar_only: bool) -> tuple[float, float, float] | None:
    kind = shape.Shape().ShapeType()
    direction = None
...
def sort_axis(
    ...
    return tuple(
        sorted(
            sequence,
            key=lambda shape: sum(value * axis for value, axis in zip(_center(shape), unit)),
            reverse=reverse,
        )
    )
