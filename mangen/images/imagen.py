...
def doscreen_impl(model, path, size, yaw=None, pitch=None, triedron=False):
	...
	if yaw is None: yaw = math.pi * (7/16) + math.pi/2 
	if pitch is None: pitch = math.pi * -0.15 
	view.set_direction(
...
doscreen(model=rectangle(20, 10), path="rectangle0.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=rectangle(20, 10, wire=True), path="rectangle1.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=rectangle(20), path="rectangle2.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=rectangle(20, wire=True), path="rectangle3.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)

doscreen(model=circle(r=20), path="circle0.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=circle(r=20, wire=True), path="circle1.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=circle(r=20, angle=(deg(45), deg(90))), path="circle2.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=circle(r=20, angle=(deg(45), deg(90)), wire=True), path="circle3.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)

doscreen(model=ellipse(r1=40, r2=20), path="ellipse0.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ellipse(r1=40, r2=20, wire=True), path="ellipse1.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ellipse(r1=40, r2=20, angle=(deg(45), deg(90))), path="ellipse2.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ellipse(r1=40, r2=20, angle=(deg(45), deg(90)), wire=True), path="ellipse3.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)

doscreen(model=polygon(pnts=[(0,0), (0,40), (20,10), (10,0)]), path="polygon0.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=polygon(pnts=[(0,0), (0,40), (20,10), (10,0)], wire=True), path="polygon1.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)

doscreen(model=ngon(r=20, n=3), path="ngon0.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ngon(r=20, n=3, wire=True), path="ngon1.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ngon(r=20, n=5), path="ngon2.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ngon(r=20, n=5, wire=True), path="ngon3.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ngon(r=20, n=8), path="ngon4.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=ngon(r=20, n=8, wire=True), path="ngon5.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)

doscreen(model=textshape(text="TextShape", fontpath="../../zencad/examples/fonts/testfont.ttf", size=100), path="textshape0.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
doscreen(model=textshape(text="TextShape", fontpath="../../zencad/examples/fonts/mandarinc.ttf", size=100), path="textshape1.png", size=wsize, yaw=yaw2d, pitch=pitch2d, triedron=True)
...
doscreen(model=sphere(r=10) + cylinder(r=5, h=30, center=True) + ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="union.png", size=wsize, yaw=yaw, pitch=pitch)
doscreen(model=sphere(r=10) + cylinder(r=5, h=30, center=True) + ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="union0.png", size=wsize, yaw=yaw1, pitch=pitch1)
doscreen(model=sphere(r=10) + cylinder(r=5, h=30, center=True) + ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="union1.png", size=wsize, yaw=yaw2, pitch=pitch2)
doscreen(model=sphere(r=10) + cylinder(r=5, h=30, center=True) + ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="union2.png", size=wsize, yaw=yaw3, pitch=pitch3)

doscreen(model=sphere(r=10) - cylinder(r=5, h=30, center=True) - ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="difference.png", size=wsize, yaw=yaw, pitch=pitch)
doscreen(model=sphere(r=10) - cylinder(r=5, h=30, center=True) - ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="difference0.png", size=wsize, yaw=yaw1, pitch=pitch1)
doscreen(model=sphere(r=10) - cylinder(r=5, h=30, center=True) - ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="difference1.png", size=wsize, yaw=yaw2, pitch=pitch2)
doscreen(model=sphere(r=10) - cylinder(r=5, h=30, center=True) - ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="difference2.png", size=wsize, yaw=yaw3, pitch=pitch3)

doscreen(model=sphere(r=10) ^ cylinder(r=5, h=30, center=True) ^ ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="intersect.png", size=wsize, yaw=yaw, pitch=pitch)
doscreen(model=sphere(r=10) ^ cylinder(r=5, h=30, center=True) ^ ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="intersect0.png", size=wsize, yaw=yaw1, pitch=pitch1)
doscreen(model=sphere(r=10) ^ cylinder(r=5, h=30, center=True) ^ ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="intersect1.png", size=wsize, yaw=yaw2, pitch=pitch2)
doscreen(model=sphere(r=10) ^ cylinder(r=5, h=30, center=True) ^ ngon(r=5, n=5).extrude(30, center=True).rotateX(deg(90)), path="intersect2.png", size=wsize, yaw=yaw3, pitch=pitch3)
...
doscreen(model=[*points([(5,0,10), (5,10,10)]), cube(10).fillet(2, [(5,0,10), (5,10,10)])], path="fillet1.png", size=wsize)
doscreen(model=cube(10).fillet(2), path="fillet2.png", size=wsize)

doscreen(model=ngon(r=5, n=6), path="fillet3.png", size=wsize)
doscreen(model=[*points([(6,0,0), (-6,0,0)]), ngon(r=5, n=6).fillet(2, [(6,0,0), (-6,0,0)])], path="fillet4.png", size=wsize)
doscreen(model=ngon(r=5, n=6).fillet(2), path="fillet5.png", size=wsize)

doscreen(model=cube(10), path="chamfer0.png", size=wsize)
doscreen(model=[*points([(5,0,10), (5,10,10)]), cube(10).chamfer(2, [(5,0,10), (5,10,10)])], path="chamfer1.png", size=wsize)
doscreen(model=cube(10).chamfer(2), path="chamfer2.png", size=wsize)

pnts = points([(0,0,20), (20,0,20)])
doscreen(model=[(cylinder(r=10, h=20) - cylinder(r=5, h=20)).chamfer(r=1, refs=pnts), *pnts], path="chamfer3.png", size=wsize)
