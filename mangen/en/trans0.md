...
`Transform` represents translation, rotation and uniform scale, including reflections through signed scale; general affine transformations use `AffineTransform`.
...
## Transforming a point and obtaining the matrix

A transformation object can be applied to a point. `Transform.matrix()` returns a numeric 4×4 matrix, evaluating the necessary dependencies.
